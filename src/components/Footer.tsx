...
// Allowlist for project scopes — kept in sync with security.ts
...
export function Footer() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ── Rate limiting ────────────────────────────────────────────────────────
    const { allowed, retryAfterMs } = contactFormRateLimiter.check();
    if (!allowed) {
      const wait = retryAfterMs ? formatRetryAfter(retryAfterMs) : 'a moment';
      alert(`You've submitted too many requests. Please wait ${wait} before trying again.`);
      return;
    }
    ...
    if (!validation.valid) {
      ...
      alert(firstError ?? 'Please fix the form errors before submitting.');
      return;
    }
    ...
    try {
      ...
      const safeData = {
        ...
        phone:       sanitizeString(formData.phone, 30),
        budget:      sanitizeString(formData.budget, 100),
        timeline:    sanitizeString(formData.timeline, 100),
        message:     sanitizeString(formData.message, 5000),
        scopes:      selectedScopes.filter(s => ALLOWED_SCOPES_SET.has(s)),
        countryCode: sanitizeString(countryCode, 10),
        createdAt:   serverTimestamp(),
      };

      // Save to Firestore
      await addDoc(collection(db, 'contact_submissions'), safeData);

      // Send email via Web3Forms (key is a non-secret public form key)
      await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          ...
          phone:    `${safeData.countryCode} ${safeData.phone}`,
          budget:   safeData.budget,
          timeline: safeData.timeline,
          scopes:   safeData.scopes.join(', '),
          message:  safeData.message,
          subject:  `New Project Inquiry from ${safeData.name} — Oxerfy`,
        }),
      });
